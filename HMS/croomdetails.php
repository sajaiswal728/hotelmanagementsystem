...
background-color: #6534AC;
 ...
 abc{
      font-family:Agency FB; src:"font/agencyb.ttf";
	  font:Agency FB;
	  font-size:25px;
	  font-weight:"bold";
    ...
    bb{
      font-family:Agency FB; src:"font/agencyr.ttf";
	  font:Agency FB;
	  font-size:18px;
	  font-weight:"bold";
	  text-shadow:2px 5px 8px grey; 
...
<td>For Room Details Select a Hostel :</td><td>
<select name="hostelc" >';
include "conn.php";
$db->query("use hms");
...
include "conn.php";
$db->query("use hms");
$m="select Seat from seats where Hcode like '".$hostelc."'";

$st=$db->query("select Seat from seats where Hcode like '".$hostelc."'");
$i=1;
$n=1;
while($c=mysqli_fetch_assoc($st)){
$n++;
...
$usedatabase=$db->query("use hms");
 $sql3=mysqli_query($db,"select uidai from seats where Seat like '".$seat."A' ");
 $sql4=mysqli_query($db,"select uidai from seats where Seat like '0".$seat."A' ");
 $sql99=mysqli_query($db,"select uidai from seats where Seat like '".$seat."B' ");
 $sql88=mysqli_query($db,"select uidai from seats where Seat like '0".$seat."B' ");
 ...
 if(@mysqli_num_rows($result)>0){
	...
	echo "<br><table align='right'><tr><td><img src='/hms/img/".$row["uidai"]."p.jpg' alt='".$row["Name"]."'><form action='student_application_form.php' method='POST'><input type='hidden' name='uidai' value='".$row['uidai']."' /><input type='submit' class='button' value='Application Form' /></form><form action='clrseat.php' method='POST'><input type='hidden' name='uidai' value='".$row['uidai']."' /><input type='submit' class='button' value='Clear this Seat' /></form></td></tr></table><br>Hostel : ".$row['Hostel']."<br>Room Number : ".$seat."<br>Name : ".$row["Name"]."<br>Phone : ".$row["Mobile"]."<br>Email : ".$row["email"]."<br>UIDAI : ".$row["uidai"]."<br>Address : ".$row["Address"]."<br>Course : ".$row["course"]." ".$row2["Course_name"]."(".$row2["Duration"]." years )<br>Batch : ".$row["Batch"]."<br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br>";
	}	
}
...
while(@$row99=mysqli_fetch_assoc($sql99))
 ...
 if(@mysqli_num_rows($result9)>0){
	...
	$row2=mysqli_fetch_assoc($sql2);
	echo "<br><table align='right'><tr><td><img src='/hms/img/".$row9["uidai"]."p.jpg' alt='".$row9["Name"]."'><form action='student_application_form.php' method='POST'><input type='hidden' name='uidai' value='".$row9['uidai']."' /><input type='submit' class='button' value='Application Form' /></form><form action='clrseat.php' method='POST'><input type='hidden' name='uidai' value='".$row99['uidai']."' /><input type='submit' class='button' value='Clear this Seat' /></form></td></tr></table><br>Hostel : ".$row9['Hostel']."<br>Room Number : ".$seat."<br>Name : ".$row9["Name"]."<br>Phone : ".$row9["Mobile"]."<br>Email : ".$row9["email"]."<br>UIDAI : ".$row9["uidai"]."<br>Address : ".$row9["Address"]."<br>Course : ".$row9["course"]." ".$row2["Course_name"]."(".$row2["Duration"]." years )<br>Batch : ".$row9["Batch"]."<br><br><br><br><br><br>";
	}	
}
...
while(@$row88=mysqli_fetch_assoc($sql8))
 {	 
 $sql="SELECT * FROM students WHERE uidai='".$row8["uidai"]."'";
...
echo"<div id='sandeep'>".$usc["count(serial)"]." Complaints Not Taken under Cosideration(or solved)<br>".$upc["count(serial)"]." Complaints under process<br><q><a href='acomplaints.php'>Check out Complaints</a></q></div>";}
else
	echo"<div id='sandeep'>No Complaints<br><a href='acomplaints.php'>Check Complaints Section</a></div>";
